...
| `1.0.12` | 打了 `v1.0.12` 这样的标签时产生，每个版本一个标签 |
| `sha-291a555` | 对应某一次提交，想固定到某个具体构建就用它 |
...
```bash
ver=1.0.12       # 按需替换为实际版本；amd64 / arm64 按机器架构选择
...
列表按每页 20 条分页（可切换 10 / 20 / 50 / 100 / 200），关键词与类型 / 用途 / 状态筛选、
翻页都在浏览器里完成，不刷新页面。
...
| 固定密钥 | 请求头（默认 `X-F2A-Token`）或 URL 参数 `?token=` 中的值必须与密钥一致。URL 参数会出现在反向代理的访问日志里，发送方支持自定义请求头时应优先用它。 |
...
列表按每页 50 条分页（可切换 10 / 20 / 50 / 100 / 200），筛选与翻页同样在浏览器里完成。
...
等待时间按 `退避基数 × 2^(次数-1)` 计算，上限 1 小时（「退避基数」可填 1–3600 秒，「最大重试次数」
可填 1–100，都在「设置」中修改）。
进程重启后会自动接管遗留的待投递记录。失败或已放弃的记录可在详情页点击「重新投递」重新入队。

列表按每页 50 条分页（可切换 10 / 20 / 50 / 100 / 200），可以翻上 / 下一页、跳到首页或末页，
也可以填页码直接跳。筛选条件跟着页码走：带条件的地址可以直接收藏或分享，翻页时筛选不会丢。
...
- 后台写操作额外校验 `Origin` / `Referer`（在 SameSite 之外的第二层防护）。认可范围：同源请求、「回调基址」的
  ...
  设置页提供开关，可整体关闭该校验。
- 登录失败 5 次锁定 5 分钟，按来源 IP 计数。反向代理后面需要先在「设置 → 反向代理」里填受信代理，
  否则所有请求都算作代理这一个来源，任何人失败 5 次都会把管理员一起锁住。
...
- **来源 IP 默认取直连对端地址**：`X-Forwarded-For` 可伪造，一律不采纳。位于反向代理后面时，
...
需要固定版本时，把 `latest` 换成 `1.0.12` 或 `sha-291a555` 这类标签，升级时再手动修改。
