...
| `1.0.9` | 打了 `v1.0.9` 这样的标签时产生，每个版本一个标签 |
| `sha-39b0bd3` | 对应某一次提交，想固定到某个具体构建就用它 |
...
```bash
ver=1.0.9        # 换成你要的版本；amd64 / arm64 按机器选
...
| --- | --- |
| 默认报文体模板 | 报文体模板 |
| 默认主题模板 | 邮件主题模板（目标若是内置渠道，它就是标题） |

典型场景：上游推过来的是 `{"title":"…","content":"…"}`，在这个源上写 `{{.Payload.title}}` /
`{{.Payload.content}}`，之后每条规则、每个目标（邮件、钉钉、飞书…）拿到的就都是干净的标题和正文，
不用每条规则各抄一遍。规则里自己填了模板以规则为准；投递日志的「原始报文（入站）」仍然记录入站原文。
...
规则里留空的模板会退回[接收源上的默认模板](#接收默认模板)，两者都没有就原样透传。
...
投递日志的时间、源/规则的创建时间、模板里的 `{{.Now}}`、以及日志行的时间戳，都按「设置 → 时区」
里填的 IANA 时区显示（例如 `Asia/Shanghai`）。

- 留空 = 跟随进程的系统时区。**Docker / systemd 里默认是 UTC**，所以看到的时间会比北京时间早 8 小时 ——
  这就是「消息时间不对」的常见原因。
- 库里存的是 Unix 时间戳，与时区无关：改这个只影响显示，不会动已有数据，也不影响投递本身。
- 换一种做法是在容器 / systemd 单元里设 `TZ=Asia/Shanghai`，那样「跟随系统」就等于该时区；
  两个都设时以设置页为准。
...
另外两个只在**运行期**读、不写进数据库的环境变量（跟上面的引导值不是一回事）：

| 变量 | 默认 | 说明 |
| --- | --- | --- |
| `F2A_TURNSTILE` | 空 | 设成 `off` 时强制关闭人机校验（被盾挡在门外时的救命开关），需要重启生效。 |
| `F2A_TURNSTILE_ENDPOINT` | Cloudflare 官方地址 | 服务端校验请求发往哪里；要经由自己的中转（或测试）时改它。 |
...
不做合并。导出文件不含管理员账号、密码哈希、监听端口、代理、时区、外观和 Turnstile 密钥，
也不含数据库主键（规则通过下标引用源），换一台机器导入不会串号。
文件里带的设置项只有回调基址和「投递与日志」的四项参数（重试次数、退避基数、报文上限、日志保留天数），
导入时覆盖本机同名项，文件里没带的设置项保持原样。整份文件先校验后落库，任何一处不合法都当作没导入过。
...
- 后台的写操作额外校验 `Origin` / `Referer`（SameSite 之外的双保险）。认可的范围：同源请求、「回调基址」的
  主机名、设置页「允许的 Origin / Referer」里列出的地址；两个头都不带的请求（curl、脚本）不校验。
  设置页有开关，可以整体关掉这个校验。
- 登录失败 5 次锁定 5 分钟。
- 「设置 → 安全设置」可以开启 **Cloudflare Turnstile** 人机校验，登录时先过盾再验密码；
  服务端会拿浏览器给的 token 去 Cloudflare 再校验一次（只靠前端控件可以被绕过）。**默认关闭**，
  关着的时候登录页不加载任何外部脚本。
...
想固定版本就把 `latest` 换成 `1.0.9` 或 `sha-39b0bd3` 这样的标签，要升级时再手动改。
...
2. 到「设置 → 跨站请求校验」的允许列表里加上浏览器访问用的地址，一行或用逗号分隔都行，例如
   `https://hooks.example.com`（只写主机名则不限协议）。代理已经改写了 Host、进不去后台时，
   先用 `IP:端口` 直接打开后台改这一项；实在不想校验也可以关掉那个开关。
...
### 开了 Turnstile 之后登录进不去

多半是密钥填错，或者服务器 / 浏览器访问不到 `challenges.cloudflare.com`。人机校验是登录前的第一道，
过不去就进不了后台，所以留了个不用进后台的开关：**用 `F2A_TURNSTILE=off` 重启进程**，校验立刻失效
（也可以先删掉数据库里 `turnstile_enabled` 这个键）。进去之后重新核对站点密钥 / 密钥，
或干脆关掉这个开关。

只勾开关不填密钥是保存不了的 —— 那种状态会把登录挡死，所以校验时会直接拒绝。
