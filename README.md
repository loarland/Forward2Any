...
| `1.0.7` | 打了 `v1.0.7` 这样的标签时产生，每个版本一个标签 |
| `sha-acecade` | 对应某一次提交，想固定到某个具体构建就用它 |
...
```bash
ver=1.0.7        # 换成你要的版本；amd64 / arm64 按机器选
...
想固定版本就把 `latest` 换成 `1.0.7` 或 `sha-acecade` 这样的标签，要升级时再手动改。
