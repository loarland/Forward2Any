...
go build -o "$WORK/w2a" ./cmd/w2a

W2A_DATA_DIR="$WORK/data" \
W2A_PORT="$APP_PORT" \
W2A_ADMIN_USER="$ADMIN_USER" \
W2A_ADMIN_PASSWORD="$ADMIN_PASS" \
W2A_BASE_URL="$BASE" \
  "$WORK/w2a" > "$WORK/app.log" 2>&1 &
PIDS+=("$!")
...
grep -q 'X-W2A-Hops: gh-e2e' "$RECEIVED" || fail "转发请求缺少跳链头"
...
curl -s -o /dev/null -X POST -H 'X-W2A-Token: e2e-secret' \
